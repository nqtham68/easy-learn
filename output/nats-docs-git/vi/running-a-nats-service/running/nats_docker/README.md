...
source_url: https://docs.nats.io/running-a-nats-service/running/nats_docker
title: NATS và Docker
translated: true
...
NATS server được cung cấp dưới dạng Docker image trên [Docker Hub](https://hub.docker.com/_/nats/) và có thể chạy bằng Docker daemon. Image này cực kỳ nhẹ, dưới 10 MB.
...
[Synadia](https://www.synadia.com?utm_source=nats_docs&utm_medium=nats) là đơn vị chủ động duy trì và hỗ trợ Docker image này.

### Nightly

> 🇬🇧 *The nightly build container can be found [here](https://hub.docker.com/r/synadia/nats-server)*

Container từ bản build nightly có thể tìm thấy [tại đây](https://hub.docker.com/r/synadia/nats-server).

### Sử dụng

> 🇬🇧 *To use the Docker container image, install Docker and pull the public image:*

Để sử dụng Docker container image, cài Docker rồi pull image công khai:
...
Mặc định, NATS server mở các port (cổng kết nối) sau:

* 4222 dành cho client.
* 8222 là HTTP management port để báo cáo thông tin.
* 6222 là routing port cho cluster (cụm nhiều server chạy chung).

> 🇬🇧 *The default ports may be customized by providing either a -p or -P option on the `docker run` command line.*

Các port mặc định có thể tùy chỉnh bằng tùy chọn `-p` hoặc `-P` trên dòng lệnh `docker run`.

> 🇬🇧 *The following steps illustrate how to run a server with the ports exposed on a `docker network.`*

Các bước sau minh họa cách chạy server với các port được expose trên `docker network.`.

> 🇬🇧 *First, create the `docker network` 'nats':*

Trước tiên, tạo `docker network` 'nats':
...
Đầu tiên, chạy server với các port được expose trên `docker network` 'nats':
...
Tiếp theo, khởi động các server bổ sung và trỏ chúng về seed server để chúng tạo thành một cluster:
...
**LƯU Ý:** Docker image bảo vệ các route bằng credential (thông tin đăng nhập), do đó cần cung cấp chúng như trên. Tham khảo [cấu hình Docker image](https://github.com/nats-io/nats-docker/blob/6fb8c05311bb4d1554390f66abb0a5ebef1e1c9d/2.1.0/scratch/amd64/nats-server.conf#L13-L19) để biết thêm chi tiết.
...
Để xác minh các route đã kết nối, gửi request đến monitoring endpoint (địa chỉ API cụ thể) trên `/routez` và kiểm tra xem có đúng 2 route:
...
### Tạo NATS Cluster với Docker Compose
...
Tạo cluster bằng Docker Compose cũng rất đơn giản. Dưới đây là ví dụ dùng network 'nats' để tạo một full mesh cluster.
...
Chạy Docker Compose để tạo cluster sử dụng network 'nats':
...
Bây giờ hãy thử: tạo subscription trên một node (một server trong cluster) và publish từ node khác. Message sẽ được nhận bình thường.
...
Dừng seed node — node đang giữ subscription — sẽ kích hoạt failover tự động sang các node còn lại:
...
Sau khi kết nối lại, việc publish tiếp tục hoạt động bình thường:
...
Xem [hướng dẫn NATS Docker](./nats-docker-tutorial.md) để biết thêm cách sử dụng NATS server Docker image.
