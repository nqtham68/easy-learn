...
source_url: https://docs.nats.io/running-a-nats-service/configuration/leafnodes/jetstream_leafnodes
title: Leaf Nodes
translated: true
...
Để xem demo đầy đủ về tính năng này, hãy xem [video](https://youtu.be/0MkS_S7lyHk) của chúng tôi.
...
Một trong những trường hợp sử dụng của NATS server được cấu hình làm [leaf node](.) là cung cấp mạng NATS cục bộ ngay cả khi mất kết nối đến hub hoặc cloud. Để hỗ trợ kịch bản mất kết nối này với JetStream, các JetStream island độc lập cũng được hỗ trợ và có thể truy cập qua cùng một mạng NATS.
...
Vấn đề chung khi có nhiều JetStream độc lập mà cùng một client có thể truy cập là cần phân biệt chúng. Ví dụ, xét một leaf node có JetStream không phân cụm trên mỗi server. Bạn kết nối vào một trong số đó, nhưng JetStream nào sẽ phản hồi khi bạn dùng JetStream API `$JS.API.>`?
...
Để phân biệt giữa các server, tùy chọn `domain` được thêm vào khối cấu hình JetStream. Khi sử dụng, cần tuân theo các quy tắc sau: Mọi server trong cluster (cụm nhiều server chạy chung) và super cluster đều phải có cùng tên domain. Điều này có nghĩa là tên domain chỉ có thể thay đổi giữa hai server nếu chúng kết nối qua leaf node. Kết quả là JetStream API `$JS.API.>` cũng sẽ có sẵn dưới tên phân biệt `$JS.<domain>.API.>`. Tất nhiên, tên domain phải là duy nhất.
...
Có những lý do để kết nối system account ở cả hai đầu của leaf node. Thông thường bạn không muốn kết nối system account của cloud và thiết bị edge, nhưng có thể muốn kết nối khi lý do duy nhất ngăn bạn dùng super cluster là các quy tắc firewall.

> 🇬🇧 *The benefits are:*

Lợi ích bao gồm:

* Giám sát tất cả nats-server đang kết nối
* nats-account-resolver hoạt động trên toàn bộ mạng
* Mở rộng JetStream cluster
...
Khi `domain` được đặt, traffic liên quan đến JetStream trên system account sẽ bị chặn. Đây chính là nguyên nhân khiến JetStream không được mở rộng.
...
Ngoài ra, traffic trên `$JS.API.>` cũng bị chặn. Điều này khiến client sử dụng JetStream cục bộ có sẵn trên nats-server mà chúng đang kết nối. Để giao tiếp với JetStream khác, cần chỉ định prefix riêng của domain đó `$JS.<domain>.API`.
...
Lưu ý rằng mỗi domain là một namespace độc lập. Trong cùng một account, hoàn toàn hợp lệ khi dùng cùng tên stream (luồng message lưu trữ liên tục) ở các domain khác nhau.
...
Ngoài ra, luồng message thông thường không bị giới hạn. Do đó, nếu cùng một subject (chuỗi định danh message (giống topic)) được đăng ký bởi các stream khác nhau trong cùng account ở các domain khác nhau, miễn là leaf node cơ bản đang kết nối tại thời điểm đó, mỗi stream đều sẽ lưu message. Có thể giải quyết bằng cách dùng cùng account nhưng subject khác nhau ở mỗi domain, hoặc dùng account khác nhau ở mỗi domain, hoặc [cô lập account](https://youtu.be/0MkS_S7lyHk?t=1151) được dùng trong leaf node.
...
Dưới đây là config (cấu hình) cần thiết để kết nối hai server có bật JetStream qua leaf node. Trong ví dụ này, system account được kết nối với mục đích minh họa (không bắt buộc).
...
Vì system account được kết nối, bạn có thể lấy báo cáo JetStream server từ cả hai server.
...
Tạo một stream tên `test` đăng ký subject `test` trong JetStream domain mà chương trình đang kết nối. Kết quả là stream này sẽ được tạo trong domain hub, tức là domain của server đang lắng nghe trên `localhost:4222`.
...
Để tạo stream trong domain khác trong khi đang kết nối ở nơi khác, chỉ cần cung cấp tham số `js-domain`. Trong khi vẫn kết nối đến cùng server như trước, stream giờ sẽ được tạo trong `leaf`.
...
Publish một message để có dữ liệu truy xuất.
...
Vì cả hai stream đều đăng ký cùng subject, mỗi stream hiện báo cáo một message. Điều này nhằm minh họa vấn đề. Để tránh, hãy dùng subject khác nhau, account khác nhau, hoặc một account được cô lập.
...
### Sao chép giữa các domain qua `source` hoặc `mirror`
...
Để sao chép stream từ domain này sang domain khác, hãy chỉ định JetStream domain khi tạo `mirror`. Nếu muốn kết nối leaf đến hub và nhận lệnh ngay cả khi leaf node offline, cách tốt nhất là mirror một stream nằm trong hub.
...
Tương tự, nếu muốn tổng hợp các stream nằm ở bất kỳ số lượng leaf node nào, hãy dùng `source`. Nếu các stream ở mỗi leaf phục vụ cùng mục đích, khuyến nghị tổng hợp chúng tại hub để xử lý qua `source`.
...
`source` cũng như `mirror` đều tạo bản sao của các message. Sau khi sao chép, việc truy cập dữ liệu không phụ thuộc vào trạng thái kết nối của leaf node. Cách này cũng tránh phải chạy một chương trình riêng biệt. Đây là cách được khuyến nghị để trao đổi dữ liệu persistent giữa các domain.
...
### Import giữa các account và domain
...
Tất cả những gì trên đây đều diễn ra trong cùng một account. Để chia sẻ quyền truy cập domain giữa các account, cần sửa `account.conf` ở trên và khởi động lại hoặc reload server. Ví dụ này export consumer (bên xử lý dữ liệu từ stream) và `FC` API cùng với delivery subject được sử dụng bởi push consumer nội bộ được tạo bởi `source` và `mirror`.
...
Để hỗ trợ ví dụ khác về cách chia sẻ durable pull consumer cho client truy cập giữa các domain và account, `NEXT` và `ACK` API cũng được export.
...
Khi import, JetStream API prefix `$JS.hub.API` được đổi tên thành `JS.test@hub.API`. Điều này nhằm phân biệt JetStream nào mà client trong account đang import muốn tương tác. Khi dùng domain, khuyến nghị chung là export API riêng của domain `$JS.<domain>.API` để có thể gắn cố định export vào một domain cụ thể.

> 🇬🇧 *Furthermore, the delivery subject is extended on import. This is to allow for easier export into multiple accounts.*

Ngoài ra, delivery subject được mở rộng khi import, để dễ dàng export vào nhiều account hơn.
...
Ví dụ này cũng chỉ export mức tối thiểu cần thiết. Có thể cấp quyền truy cập toàn bộ consumer API `$JS.hub.API.CONSUMER.>` hoặc toàn bộ API trong một domain `$JS.hub.API.>` hoặc toàn bộ API `$JS.API.>` bất kể client import kết nối ở đâu.
...
Sau khi server được khởi động lại hoặc reload, có thể tạo `mirror` như sau (tương tự với `source`): Khi import từ account khác, prefix đã đổi tên `JS.acc@hub.API` được cung cấp. Ngoài ra, tên delivery subject được mở rộng để bao gồm cả domain và stream đang import, giúp nó là duy nhất cho lần import đó. Nếu mọi delivery prefix đều tuân theo pattern `<static type>.<exporting account>.<exporting domain>.<importing account>.<importing domain>.<importing stream name>`, có thể tránh được xung đột do nhiều lần import gây ra.
...
Kiểm tra tiếp theo cho thấy message được lưu trong stream aggregate ở account `ACC` đã được sao chép sang stream mới trong account `IMPORTER`.
...
#### Truy cập trực tiếp durable pull consumer
...
`accounts.conf` đã sửa đổi cũng bao gồm một import riêng cho pull consumer hiện có. Hãy tạo một consumer tên `dur` trong stream `aggregate-test-leaf` thuộc account `acc`.
...
Để truy xuất message được lưu trong domain `hub` bằng `nats` trong khi đang kết nối đến leaf node, cung cấp tên stream, tên durable và API prefix `JS.acc@hub.API` đúng.
...
Cách này hoạt động tương tự khi bạn viết client của riêng mình. Để tránh chờ ack timeout, một message mới được gửi trên `test` từ đó được sao chép vào `aggregate-test-leaf`.
...
Client kết nối đến leaf node và nhận message vừa gửi.
...
Ở đây API prefix được truyền vào bằng cách đặt tùy chọn `nats.APIPrefix("JS.acc@hub.API")` khi lấy JetStream object. Vì quyền truy cập API bị giới hạn, lời gọi subscribe cung cấp tùy chọn `nats.Bind("aggregate-test-leaf", "dur")` để ngăn việc tự động suy luận tên stream và durable.
...
Một push subscriber cũng cần cấu hình tương tự. Nó sẽ cần subject `ACK`. Tuy nhiên, thay vì export/import subject `NEXT`, cần sử dụng delivery subject được chỉ định cho source/mirror.
...
[JetStream Leaf Nodes demo](https://github.com/nats-io/jetstream-leaf-nodes-demo) và [video demo kèm theo](https://www.youtube.com/watch?v=0MkS_S7lyHk).
