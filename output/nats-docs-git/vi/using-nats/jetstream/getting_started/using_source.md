...
source_url: https://docs.nats.io/using-nats/jetstream/getting_started/using_source
title: Sử Dụng Source Code
translated: true
...
Một trong những mục tiêu thiết kế của JetStream là tích hợp gốc với core NATS. Dù chúng tôi chắc chắn sẽ bổ sung các tiện ích cú pháp vào client sau này, trong phiên bản tech preview này chúng ta sẽ dùng NATS thuần túy.

> 🇬🇧 *You will need a copy of the nats-server source locally and will need to be in the jetstream branch.*

Bạn cần có source code của nats-server trên máy và phải ở nhánh `jetstream`.
...
Khi khởi động server, dùng flag `-js` để cấu hình server sử dụng bộ nhớ và ổ đĩa hợp lý. Đây là ví dụ chạy trên máy của tôi. JetStream (luồng message lưu trữ liên tục) mặc định dùng 1TB ổ đĩa và 75% RAM khả dụng.
...
Bạn có thể thay đổi thư mục lưu trữ nếu muốn.
...
Các tùy chọn này cũng có thể khai báo trong config (cấu hình) file:
