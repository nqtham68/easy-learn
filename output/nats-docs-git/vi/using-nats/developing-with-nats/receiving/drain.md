...
source_url: https://docs.nats.io/using-nats/developing-with-nats/receiving/drain
title: Drain Message Trước Khi Ngắt Kết Nối
translated: true
translated_at: '2026-05-17T00:00:00Z'
---

# Drain Message Trước Khi Ngắt Kết Nối
...
Tính năng này cho phép drain (xả hết) các connection hoặc subscription trước khi đóng kết nối. Thao tác đóng connection (dùng `close()`) hay hủy đăng ký subscription thường được xử lý ngay lập tức — khi đó library sẽ dừng mọi message (gói dữ liệu được gửi đi) còn trong queue (hàng đợi message) hoặc cache chờ gửi tới subscriber. Khi drain một subscription hoặc connection, toàn bộ message đang trong flight hoặc đang được cache/chờ xử lý sẽ được hoàn tất trước khi đóng.
...
Drain cung cấp cho client (bên gọi phía người dùng) dùng queue subscription một cách tắt ứng dụng mà không mất message nào. Client có thể khởi động thành viên queue mới, drain và tắt thành viên queue cũ — tất cả mà không làm mất message nào gửi tới client cũ. Nếu không có drain, message có thể bị mất do vấn đề thời điểm delivery.

> 🇬🇧 *The libraries can provide drain on a connection or on a subscriber, or both.*

Các library có thể hỗ trợ drain trên connection, trên subscriber (bên đăng ký nhận message), hoặc cả hai.

> 🇬🇧 *For a connection the process is essentially:*

Với connection, quy trình drain diễn ra như sau:

1. Drain tất cả subscription
2. Ngừng publish message mới
3. Flush toàn bộ message đã publish còn lại
4. Đóng kết nối

> 🇬🇧 *The API for drain can generally be used instead of close:*

API drain thường có thể dùng thay cho close:

> 🇬🇧 *As an example of draining a connection:*

Ví dụ drain một connection:
...
Cơ chế drain cho subscription đơn giản hơn:

1. Hủy đăng ký (unsubscribe)
2. Xử lý toàn bộ message đang được cache hoặc đang trong flight
3. Dọn dẹp tài nguyên

> 🇬🇧 *The API for drain can generally be used instead of unsubscribe:*

API drain thường có thể dùng thay cho unsubscribe:
...
Vì drain có thể liên quan đến việc message vẫn tiếp tục chảy tới server, kết hợp với flush và xử lý message theo kiểu async (bất đồng bộ), timeout cho drain thường nên đặt cao hơn so với timeout cho một request-reply thông thường.
