...
source_url: https://docs.nats.io/using-nats/developing-with-nats/js/object
title: Object Store
translated: true
...
> 🇬🇧 *The Object Store allows you to store data of any (i.e. large) size by implementing a chunking mechanism, allowing you to for example store and retrieve files (i.e. the object) of any size by associating them with a path and a file name (i.e. the key). You obtain a ObjectStoreManager object from your JetStream [context](./context.md).*

Object Store (bộ lưu trữ đối tượng) cho phép lưu trữ dữ liệu với kích thước tùy ý (kể cả rất lớn) bằng cơ chế chia nhỏ (chunking). Nhờ đó bạn có thể lưu và truy xuất file (tức là object) ở bất kỳ kích thước nào, thông qua một đường dẫn và tên file (tức là key). Bạn lấy đối tượng `ObjectStoreManager` từ JetStream [context](./context.md) của mình.
...
Xem thêm tại [jetstream/object.go](https://github.com/nats-io/nats.go/blob/main/jetstream/object.go)
