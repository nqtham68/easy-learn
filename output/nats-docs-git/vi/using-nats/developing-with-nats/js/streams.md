...
source_url: https://docs.nats.io/using-nats/developing-with-nats/js/streams
title: Quản lý Stream và Consumer
translated: true
translated_at: '2026-05-17T00:00:00Z'
---

# Quản lý Stream và Consumer
...
Stream (luồng message lưu trữ liên tục) và durable consumer (bên xử lý dữ liệu từ stream) có thể được định nghĩa ngoài ứng dụng thông qua quản trị — thường dùng NATS CLI Tool — khi đó ứng dụng chỉ cần biết tên của các durable consumer muốn sử dụng. Ngoài ra, bạn cũng có thể quản lý stream và consumer trực tiếp trong code.
...
Các thao tác quản lý stream phổ biến:

* Thêm stream. Đây là thao tác idempotent — nếu stream chưa tồn tại, nó sẽ được tạo mới; nếu đã tồn tại, thao tác thêm chỉ thành công khi stream hiện có khớp chính xác với các thuộc tính được chỉ định trong lệnh gọi 'add'.
* Xóa stream.
* Purge stream (xóa toàn bộ message đang lưu trong stream).
* Lấy hoặc xóa một message cụ thể khỏi stream theo sequence number.
* Thêm, cập nhật hoặc xóa consumer.
* Xem thông tin và thống kê về stream/consumer/account. Lấy, xóa hoặc xem thông tin từng message đang lưu trong stream.
